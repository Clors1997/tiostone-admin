<template>
  <div>
    <p class="header">
      <span>
        <a-input-search placeholder="PO Number" style="width: 200px" @search="onSearch"/>
      </span>
      <span>
      </span>
    </p>
    ...
    >
      ...
      <template slot="action" slot-scope="record">
        <a-popconfirm
          ...
          okText="yes"
          cancelText="no"
          @confirm="() => onDelete(record.invoice_id, record.record_num)"
        >
...
<script>
import { r_discount_record, r_discount_record_product, d_discount_record } from "@/api/discount_record.js";
...
export default {
  ...
  created() {
    this.columns = [
      { width: 30, scopedSlots: { customRender: "plus" } },
      ...
      { title: "Total(HKD $)", width: 150, dataIndex: "record_total" },
  ...
  methods: {
    ...
    getTableData(pagenum, size) {
        ...
        r_discount_record(pagenum, size, 0,this.search)
          .then(res => {
    ...
    onDelete(invoice_id, record_num){
      console.log(invoice_id, record_num);
      d_discount_record(invoice_id, record_num)
        .then(res => {
          console.log(res);
